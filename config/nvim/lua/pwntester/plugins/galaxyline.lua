return function()
  ...
  gl.short_line_list = vim.tbl_filter(function(item)
    return item ~= "octo"
  end, vim.fn.deepcopy(g.special_buffers))
  ...
  gls.left[3] = {
    GitHubRepo = {
      ...
      icon = " ",
      condition = function()
        if vim.tbl_contains(gl.short_line_list, vim.bo.ft) then
          return
        end
  ...
  gls.right[1] = {
    FileName = {
      provider = function()
        ...
        if
          vim.startswith(bufname, "octo:")
          or vim.startswith(bufname, "codeql:")
          or vim.startswith(bufname, "docker:")
        then
          return bufname
        else
