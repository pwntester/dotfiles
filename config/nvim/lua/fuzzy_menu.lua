...
local function apply_mappings(mappings)
  ...
  for k, v in pairs(mappings) do
    ...
    if opts.buffer then
      ...
      opts.noremap = true
      rhs = format("<cmd>lua require'fuzzy_menu'.key_callbacks[%d][%q]()<cr>", bufnr, ekey)
      api.nvim_buf_set_keymap(bufnr, mode, lhs, rhs, opts)
    else
      local ekey = tohex(lhs:lower())
      key_callbacks[-1][ekey] = rhs
      opts.noremap = true
      rhs = format("<cmd>lua require'fuzzy_menu'.key_callbacks[-1][%q]()<cr>", ekey)
...
local function floating_fuzzy_menu(options)
  ...
  local width_per = options.width_per or 0.6
  local height_per = options.height_per or 0.6
  local virtual_text = options.virtual_text or ''

  local bufnr, winnr = require('window').floating_window({
	border = true;
	width_per = width_per;
  ...
  if leave_empty_line then
    visible_entry_count = min(height - 2, entry_count)
  end
  ...
  local function redraw()
	...
	if prompt_position == 'bottom' then
		...
		api.nvim_buf_add_highlight(bufnr, ns, 'Visual', height - highlighted_line - 1 - offset, 0, -1)
		api.nvim_buf_add_highlight(bufnr, ns, 'Question', height - highlighted_line - 1 - offset, 0, -1)
    elseif prompt_position == 'top' then
        if leave_empty_line then
  ...
  vim.cmd(format("autocmd TextChangedI <buffer=%d> lua require'fuzzy_menu'.popup_callback()", bufnr))
